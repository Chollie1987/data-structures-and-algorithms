...
const updateNumbers = (obj) => {
  const newNumbers = Object.entries(obj).map(([name, phoneNumber]) => `${name}: ${phoneNumber}`);
  return newNumbers;
};
...
const hasChildrenValues = (arr, character) => {
  let characterObj = arr.find(char => char.name === character);
  let values = Object.values(characterObj);
  return values.includes(characterObj.children);
};
