...
const count = (target, input) => {
  const arrayCounted = input.map((array) => {
  ...
  const total = arrayCounted.reduce((accumulator, number) => {
...
const totalSum = (input) => {
  const integerArr = input.reduce((accumulator, arr) => {
    const sum = arr.reduce((firstAccumulator, number) => {
      return firstAccumulator + number;
    }, 0);
...
let findMaleAndFemale = (data) => {
  ...
  const chosenCharacter = characters.filter(characters => {
    return characters.gender === 'male' || characters.gender === 'female';
  });
  const charctersName = chosenCharacter.map(characters => characters.name);
  const combinedNames = charctersName.join(' and ');
